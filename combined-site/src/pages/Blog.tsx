import React, { useState } from "react";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import DesktopPageWrapper from "../components/DesktopPageWrapper";
import MobBanner from "../components/MobBanner";
import MobServiceCategories from "../components/MobServiceCategories";
import MobFooter from "../components/MobFooter";


const DESIGN_HEIGHT = 4714;

type Ecosystem = "Products" | "Services" | "Partners";
type Expertise = "Research" | "Design" | "Development" | "Creative" | "Marketing";

const ecosystemData: Record<Ecosystem, Expertise[]> = {
  Products: ["Research", "Design", "Development"],
  Services: ["Research", "Design", "Creative", "Marketing"],
  Partners: ["Research", "Development", "Marketing"],
};

const Badge = ({ children }: { children: React.ReactNode }) => (
  <div className="inline-flex h-[38px] items-center justify-center rounded-full border border-[#202833] px-[22px] text-[14px] font-medium uppercase tracking-[3px] text-[#202833]">
    {children}
  </div>
);

const PillButton = ({
  label,
  active,
  onClick,
  width = 194,
}: {
  label: string;
  active?: boolean;
  onClick: () => void;
  width?: number;
}) => (
  <button
    onClick={onClick}
    className={`flex items-center justify-center gap-[26px] rounded-[18px] border-0 transition-colors ${
      active ? "bg-[#0161fe] text-white" : "bg-white text-[#0161fe]"
    }`}
    style={{ width, height: 78 }}
  >
    <span
      className={`flex items-center justify-center rounded-full text-[18px] ${
        active ? "bg-white/25" : "bg-[#0161fe] text-white"
      }`}
      style={{ width: 42, height: 42 }}
    >
      ↯
    </span>
    <span className="text-[20px] leading-[24px]">{label}</span>
  </button>
);

const BlogDesktop = () => {
  const [activeEcosystem, setActiveEcosystem] = useState<Ecosystem>("Services");
  const [activeExpertise, setActiveExpertise] = useState<Expertise>("Design");
  const visibleExpertise = ecosystemData[activeEcosystem];

  const chooseEcosystem = (item: Ecosystem) => {
    setActiveEcosystem(item);
    setActiveExpertise(ecosystemData[item][0]);
  };

  return (
    <>
      <DesktopPageWrapper designHeight={DESIGN_HEIGHT}>
        <div
          id="_236_945__Frame_1171277066"
          className="absolute h-[4607px] w-[1420.00px] flex flex-col justify-start items-start flex-nowrap gap-[50px] left-[10.00px] top-[107.00px]"
        >
          <div
            id="_236_946__Section"
            className="relative overflow-hidden bg-[rgba(246,247,249,1.00)] h-[822.00px] w-full rounded-[23.686382293701172px]"
          >
            <span
              id="_236_947__Heading_1___Explore_"
              className="flex justify-start text-left items-center h-[57.00px] w-[350.00px] absolute left-[42.00px] top-[51.00px]"
            >
              <span
                className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[48.0px] font-normal leading-[54.33px]"
              >
                Explore Products
              </span>
            </span>
            <span
              id="_236_948__Explore_a_wide_range"
              className="flex justify-start text-left items-center h-[57.00px] w-[498.00px] absolute left-[880.00px] top-[51.00px]"
            >
              <span
                className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[16.0px] font-normal leading-[22.00px]"
              >
                Explore a wide range of services crafted to support your vision.
                From digital design and marketing growth to industry-specific
                solutions, I provide the tools and expertise to help your
                business build, scale, and succeed.
              </span>
            </span>
            <div
              id="_236_949__Container"
              className="absolute overflow-hidden bg-[rgba(254,254,254,1.00)] h-[75.55%] w-[94.08%] rounded-[23.686382293701172px] left-[calc(100%_*_0.03)] top-[calc(100%_*_0.19)]"
            ></div>
          </div>

          <section className="w-full mt-[50px] h-[367px] rounded-[20px] bg-[#f6f7f9] pt-[50px] text-center">
            <Badge>Our Ecosystem</Badge>
            <h2 className="mt-[39px] text-[36px] font-normal leading-[43px] text-[#111]">
              Explore Our Business
              <br />
              Ecosystem
            </h2>
            <div className="mt-[37px] flex justify-center gap-[22px]">
              {(["Products", "Services", "Partners"] as Ecosystem[]).map((item) => (
                <PillButton
                  key={item}
                  label={item}
                  active={activeEcosystem === item}
                  onClick={() => chooseEcosystem(item)}
                />
              ))}
            </div>
          </section>

          <section className="w-full mt-[50px] h-[367px] rounded-[20px] bg-[#f6f7f9] pt-[50px] text-center">
            <Badge>Our Expertise</Badge>
            <h2 className="mt-[39px] text-[36px] font-normal leading-[43px] text-[#111]">
              Our Core Capabilities &amp;
              <br />
              Expertise
            </h2>
            <div className="mt-[40px] flex justify-center gap-[22px]">
              {visibleExpertise.map((item) => (
                <PillButton
                  key={item}
                  label={item}
                  active={activeExpertise === item}
                  onClick={() => setActiveExpertise(item)}
                  width={item === "Development" ? 236 : item === "Marketing" ? 204 : 188}
                />
              ))}
            </div>
          </section>

          <section
            className="w-full rounded-[20px] bg-[#f6f7f9] pt-[50px] text-center"
            style={{ height: 367 }}
          >
            <Badge>Our Solutions</Badge>
            <h2 className="mt-[32px] text-[36px] font-normal leading-[43px] text-[#111]">
              Transforming Ideas into
              <br />
              Digital Products
            </h2>
            <div className="mt-[34px] flex justify-center gap-[22px]">
              {["Website", "Mobile App", "Saas", "Poster", "Logo"].map((item, index) => (
                <button
                  key={item}
                  className={`flex items-center justify-center gap-[24px] rounded-[18px] border-0 ${
                    index === 0 ? "bg-[#0161fe] text-white" : "bg-white text-[#0161fe]"
                  }`}
                  style={{ height: 78, width: index === 0 ? 194 : index === 1 ? 188 : 150 }}
                >
                  <span
                    className={`flex items-center justify-center rounded-full text-[18px] ${
                      index === 0 ? "bg-white/25" : "bg-[#0161fe] text-white"
                    }`}
                    style={{ width: 42, height: 42 }}
                  >
                    ↯
                  </span>
                  <span className="text-[20px] leading-[24px]">{item}</span>
                </button>
              ))}
            </div>
          </section>

          <div
            id="_236_960__Big_box"
            className="relative overflow-hidden bg-[rgba(246,247,249,1.00)] h-[1973px] w-full flex flex-col justify-start items-center flex-nowrap gap-[60px] px-[49px] py-[55px] rounded-[23.686382293701172px]"
          >
            <div
              id="_236_961__Frame_129"
              className="relative h-[132.00px] w-[1322.00px] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
            >
              <div
                id="_236_962__Border"
                className="relative flex flex-row justify-center items-center flex-nowrap gap-2.5 px-5 py-[9px] rounded-[236.84442138671875px] border-[#202833ff] border-solid border-[0.9695612788200378px]"
              >
                <span
                  id="_236_963__Projects"
                  className="flex justify-start text-left items-center h-[20.00px] w-[83.00px] relative"
                >
                  <span
                    className="whitespace-nowrap bg-[rgba(32,40,51,1.00)] bg-clip-text text-transparent not-italic text-[14.0px] font-medium leading-[19.61px] tracking-[2.0719525814056396px] uppercase"
                  >
                    Projects
                  </span>
                </span>
              </div>

              <div
                id="_236_964__Frame_128"
                className="relative h-[64.00px] w-[calc(100%-0px-0px)] flex flex-row justify-start items-center flex-nowrap gap-[511px]"
              >
                <div
                  id="_236_965__Frame_126"
                  className="relative h-[44.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                >
                  <span
                    id="_236_966__Heading_2___Our_Succ"
                    className="flex justify-start text-left items-center h-[69.00px] w-[377.00px] relative"
                  >
                    <span
                      className="bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[35.0px] font-normal leading-[34.01px]"
                    >
                      Real Results. Real Impact. <br />
                      Our Success Stories.
                    </span>
                  </span>
                </div>

                <div
                  id="_236_967__Frame_127"
                  className="relative w-[393.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                >
                  <span
                    id="_236_968__Discover_how_my_UX_U"
                    className="flex justify-start text-left items-center h-[72.00px] w-[404.00px] relative"
                  >
                    <span
                      className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[16.0px] font-normal leading-[23.69px]"
                    >
                      Discover how my UX/UI design, digital marketing, and
                      equity research projects deliver practical solutions,
                      solve real challenges, and create measurable outcomes.
                    </span>
                  </span>
                </div>
              </div>
            </div>

            <div
              id="_236_969__Frame_1171276978"
              className="relative flex flex-col justify-start items-start flex-nowrap gap-[30px]"
            >
              <div
                id="_236_970__Frame_1171276976"
                className="relative h-[476.00px] w-[1323.00px]"
              >
                <div
                  id="_236_971__Frame_1171276940"
                  className="absolute h-[320.00px] w-[1320.01px] flex flex-row justify-start items-center flex-nowrap gap-5 left-[0.99px] top-[0.00px]"
                >
                  <div
                    id="_236_972__Frame_1171276973"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_973__Frame_1171276938"
                      className="absolute h-[320.00px] w-[426.67px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_974__Frame_125"
                        className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                      >
                        <div
                          id="_236_975__Image_Box"
                          className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                        >
                          <div
                            id="_236_976__Frame_103"
                            className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                          >
                            <div
                              id="_236_977__Frame_104"
                              className="relative h-[306.00px] w-full"
                            >
                              <div
                                id="_236_978__image_26"
                                className="absolute h-[222.00px] w-[180.00px] left-[calc(50%-89.62px)] top-[calc(50%-111.18px)]"
                                style={{
                                  background:
                                    "url(/assets/Home/images/image_26.png) 100% / cover no-repeat",
                                }}
                              ></div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_979__Frame_1171276936"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_980__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_981__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_982__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_983__image_10"
                            className="absolute h-[136.00px] w-[193.00px] left-[calc(50%-96.29px)] top-[calc(50%-68.18px)]"
                            style={{
                              background:
                                "url(/assets/Home/images/image_10.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_984__Frame_1171276939"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_985__Frame_125"
                      className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_986__Image_Box"
                        className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                      >
                        <div
                          id="_236_987__Frame_103"
                          className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                        >
                          <div
                            id="_236_988__Frame_104"
                            className="relative h-[306.00px] w-full"
                          >
                            <img
                              id="_236_989__Group_11"
                              src="/assets/blogs/images/group_11.svg"
                              alt="Group_11"
                              className="absolute left-[calc(100%_*_0.30)] top-[calc(100%_*_0.24)]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_236_998__Frame_1171276975"
                  className="absolute h-[136.00px] w-[1321.00px] left-[-0.50px] flex flex-row justify-start items-start flex-nowrap gap-5 top-[340.00px]"
                >
                  <div
                    id="_236_999__Frame_124"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1000__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1001__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1002__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1003__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1004__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1005__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1006__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1007__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1008__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1009__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1010__Vector"
                                  src="/assets/blogs/images/vector_14.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1011__Frame_50"
                        >
                          <div
                            id="_236_1012__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1016__Frame_1171276942"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1017__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1018__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1019__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1020__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1021__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1022__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1023__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1024__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1025__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1026__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1027__Vector"
                                  src="/assets/blogs/images/vector_15.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1028__Frame_50"
                        >
                          <div
                            id="_236_1029__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1033__Frame_1171276943"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1034__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1035__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1036__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1037__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1038__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1039__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1040__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1041__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1042__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1043__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1044__Vector"
                                  src="/assets/blogs/images/vector_16.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1045__Frame_50"
                        >
                          <div
                            id="_236_1046__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_236_1050__Frame_1171276978"
                className="relative h-[476.00px] w-[1323.00px]"
              >
                <div
                  id="_236_1051__Frame_1171276940"
                  className="absolute h-[320.00px] w-[1320.01px] flex flex-row justify-start items-center flex-nowrap gap-5 left-[0.99px] top-[0.00px]"
                >
                  <div
                    id="_236_1052__Frame_1171276973"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1053__Frame_1171276938"
                      className="absolute h-[320.00px] w-[426.67px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1054__Frame_125"
                        className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                      >
                        <div
                          id="_236_1055__Image_Box"
                          className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                        >
                          <div
                            id="_236_1056__Frame_103"
                            className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                          >
                            <div
                              id="_236_1057__Frame_104"
                              className="relative h-[306.00px] w-full"
                            >
                              <div
                                id="_236_1058__image_47"
                                className="absolute h-[81.00px] w-[366.00px] left-[calc(50%-182.62px)] top-[calc(50%-40.68px)]"
                                style={{
                                  background:
                                    "url(/assets/blogs/images/image_47.png) 100% / cover no-repeat",
                                }}
                              ></div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1059__Frame_1171276936"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1060__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1061__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_1062__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_1063__image_63"
                            className="absolute h-[76.17px] w-[271.85px] left-[calc(50%-135.72px)] top-[calc(50%-38.27px)]"
                            style={{
                              background:
                                "url(/assets/blogs/images/image_63.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1064__Frame_1171276939"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1065__Frame_125"
                      className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1066__Image_Box"
                        className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                      >
                        <div
                          id="_236_1067__Frame_103"
                          className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                        >
                          <div
                            id="_236_1068__Frame_104"
                            className="relative h-[306.00px] w-full"
                          >
                            <img
                              id="_236_1070__Vector"
                              src="/assets/blogs/images/vector_17.svg"
                              alt="Vector"
                              className="absolute left-[99.22px] top-[122.60px]"
                            />
                            <span
                              id="_236_1071__Resora"
                              className="flex justify-start text-left items-start h-[58.00px] w-[136.00px] absolute left-[176.52px] top-[126.04px]"
                            >
                              <span>
                                <span
                                  className="whitespace-nowrap bg-[rgba(31,100,255,1.00)] bg-clip-text text-transparent not-italic text-[38.86934280395508px] font-semibold"
                                >
                                  R
                                </span>
                                <span
                                  className="whitespace-nowrap bg-[rgba(31,100,255,1.00)] bg-clip-text text-transparent not-italic text-[38.86934280395508px] font-medium"
                                >
                                  e
                                </span>
                                <span
                                  className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[38.86934280395508px] font-medium"
                                >
                                  sora
                                </span>
                              </span>
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_236_1072__Frame_1171276975"
                  className="absolute h-[136.00px] w-[1321.00px] left-[-0.50px] flex flex-row justify-start items-start flex-nowrap gap-5 top-[340.00px]"
                >
                  <div
                    id="_236_1073__Frame_124"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1074__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1075__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1076__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1077__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1078__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1079__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1080__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1081__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1082__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1083__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1084__Vector"
                                  src="/assets/blogs/images/vector_18.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1085__Frame_50"
                        >
                          <div
                            id="_236_1086__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1090__Frame_1171276942"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1091__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1092__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1093__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1094__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1095__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1096__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1097__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1098__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1099__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1100__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1101__Vector"
                                  src="/assets/blogs/images/vector_19.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1102__Frame_50"
                        >
                          <div
                            id="_236_1103__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1107__Frame_1171276943"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1108__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1109__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1110__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1111__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1112__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1113__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1114__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1115__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1116__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1117__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1118__Vector"
                                  src="/assets/blogs/images/vector_20.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1119__Frame_50"
                        >
                          <div
                            id="_236_1120__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_236_1124__Frame_1171276977"
                className="relative h-[476.00px] w-[1323.00px]"
              >
                <div
                  id="_236_1125__Frame_1171276940"
                  className="absolute h-[320.00px] w-[1320.01px] flex flex-row justify-start items-center flex-nowrap gap-5 left-[0.99px] top-[0.00px]"
                >
                  <div
                    id="_236_1126__Frame_1171276973"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1127__Frame_1171276938"
                      className="absolute h-[320.00px] w-[426.67px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1128__Frame_125"
                        className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                      >
                        <div
                          id="_236_1129__Image_Box"
                          className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                        >
                         
                              <div
                                id="_236_1132__image_242"
                                className="absolute h-[199.00px] w-[184.00px] left-[calc(50%-91.62px)] top-[calc(50%-99.68px)]"
                                style={{
                                  background:
                                    "url(/assets/products/images/image_242.png) 100% / cover no-repeat",
                                }}
                              ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1133__Frame_1171276936"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1134__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1135__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_1136__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_1137__image_1"
                            className="absolute h-[133.00px] w-[168.00px] left-[calc(50%-84.14px)] top-[calc(50%-66.68px)]"
                            style={{
                              background:
                                "url(/assets/blogs/images/image_1.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1138__Frame_1171276939"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1139__Frame_125"
                      className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1140__Image_Box"
                        className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                      >
                        <div
                          id="_236_1141__Frame_103"
                          className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                        >
                          <div
                            id="_236_1142__Frame_104"
                            className="relative h-[306.00px] w-full"
                          >
                            <img
                              id="_236_1143__Group_1000008334"
                              src="/assets/blogs/images/group_1000008334.svg"
                              alt="Group_1000008334"
                              className="absolute left-[calc(100%_*_0.33)] top-[calc(100%_*_0.20)]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_236_1146__Frame_1171276975"
                  className="absolute h-[136.00px] w-[1321.00px] left-[-0.50px] flex flex-row justify-start items-start flex-nowrap gap-5 top-[340.00px]"
                >
                  <div
                    id="_236_1147__Frame_124"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1148__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1149__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1150__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1151__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1152__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1153__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1154__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1155__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1156__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1157__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1158__Vector"
                                  src="/assets/blogs/images/vector_21.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1159__Frame_50"
                        >
                          <div
                            id="_236_1160__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1164__Frame_1171276942"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1165__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1166__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1167__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1168__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1169__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1170__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1171__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1172__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1173__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1174__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1175__Vector"
                                  src="/assets/blogs/images/vector_22.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1176__Frame_50"
                        >
                          <div
                            id="_236_1177__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1181__Frame_1171276943"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1182__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1183__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1184__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1185__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1186__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1187__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1188__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1189__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1190__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1191__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1192__Vector"
                                  src="/assets/blogs/images/vector_23.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1193__Frame_50"
                        >
                          <div
                            id="_236_1194__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_236_1198__Frame_1171276979"
                className="relative h-[476.00px] w-[1323.00px]"
              >
                <div
                  id="_236_1199__Frame_1171276940"
                  className="absolute h-[320.00px] w-[1320.01px] flex flex-row justify-start items-center flex-nowrap gap-5 left-[0.99px] top-[0.00px]"
                >
                  <div
                    id="_236_1200__Frame_1171276973"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1201__Frame_1171276938"
                      className="absolute h-[320.00px] w-[426.67px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1202__Frame_125"
                        className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                      >
                        <div
                          id="_236_1203__Image_Box"
                          className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                        >
                          <div
                            id="_236_1204__Frame_103"
                            className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                          >
                            <div
                              id="_236_1205__Frame_104"
                              className="relative h-[306.00px] w-full"
                            >
                              <div
                                id="_236_1206__image_247"
                                className="absolute h-[84.00px] w-[185.00px] left-[calc(50%-92.47px)] top-[calc(50%-42.18px)] rounded-[89px]"
                                style={{
                                  background:
                                    "url(/assets/blogs/images/image_247.png) 100% / cover no-repeat",
                                }}
                              ></div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1207__Frame_1171276936"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1208__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1209__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_1210__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_1211__image_15"
                            className="absolute h-[90.00px] w-[243.00px] left-[calc(50%-121.29px)] top-[calc(50%-45.18px)]"
                            style={{
                              background:
                                "url(/assets/blogs/images/image_15.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1212__Frame_1171276939"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1213__Frame_125"
                      className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1214__Image_Box"
                        className="relative overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px]"
                      >
                        <div
                          id="_236_1215__Frame_103"
                          className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                        >
                          <div
                            id="_236_1216__Frame_104"
                            className="relative h-[306.00px] w-full"
                          >
                            <div
                              id="_236_1217__Screenshot_2025-12-1"
                              className="absolute h-[109.00px] w-[268.00px] left-[calc(50%-133.96px)] top-[calc(50%-54.18px)] rounded-[45px]"
                              style={{
                                background:
                                "url(/assets/products/images/screenshot_20251219_at_103139_pm_1.png) 100% / cover no-repeat"
                              }}
                            ></div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_236_1218__Frame_1171276975"
                  className="absolute h-[136.00px] w-[1321.00px] left-[-0.50px] flex flex-row justify-start items-start flex-nowrap gap-5 top-[340.00px]"
                >
                  <div
                    id="_236_1219__Frame_124"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1220__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1221__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1222__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1223__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1224__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1225__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1226__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1227__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1228__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1229__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1230__Vector"
                                  src="/assets/blogs/images/vector_24.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1231__Frame_50"
                        >
                          <div
                            id="_236_1232__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1236__Frame_1171276942"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1237__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1238__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1239__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1240__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1241__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1242__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1243__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1244__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1245__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1246__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1247__Vector"
                                  src="/assets/blogs/images/vector_25.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1248__Frame_50"
                        >
                          <div
                            id="_236_1249__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1253__Frame_1171276943"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1254__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1255__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1256__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1257__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1258__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1259__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1260__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1261__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1262__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1263__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1264__Vector"
                                  src="/assets/blogs/images/vector_26.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1265__Frame_50"
                        >
                          <div
                            id="_236_1266__iconamoon_profile"
                          >
                             <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_236_1270__Frame_1171276980"
                className="relative h-[476.00px] w-[1323.00px]"
              >
                <div
                  id="_236_1271__Frame_1171276940"
                  className="absolute h-[320.00px] w-[1320.01px] flex flex-row justify-start items-center flex-nowrap gap-5 left-[0.99px] top-[0.00px]"
                >
                  <div
                    id="_236_1272__Frame_1171276974"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1273__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1274__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_1275__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_1276__image-removebg-previ"
                            className="absolute h-[200.00px] w-[204.00px] left-[calc(50%-101.79px)] top-[calc(50%-100.18px)]"
                            style={{
                              background:
                                "url(/assets/products/images/imageremovebgpreview_4.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1277__Frame_1171276936"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1278__Image_Box"
                      className="absolute overflow-hidden bg-white h-[320.00px] w-[426.67px] rounded-[17.381328582763672px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_236_1279__Frame_103"
                        className="absolute h-[306.68px] w-[412.34px] left-[-0.20px] top-[-0.16px] flex flex-col justify-start items-start flex-nowrap gap-[7px] p-[7px]"
                      >
                        <div
                          id="_236_1280__Frame_104"
                          className="relative h-[306.00px] w-full"
                        >
                          <div
                            id="_236_1281__image_248"
                            className="absolute h-[200.00px] w-[210.00px] left-[calc(50%-104.79px)] top-[calc(50%-100.18px)]"
                            style={{
                              background:
                                "url(/assets/products/images/image_248.png) 100% / cover no-repeat",
                            }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1282__Frame_1171276939"
                    className="relative h-[320.00px] w-[426.67px]"
                  >
                    <div
                      id="_236_1283__Frame_125"
                      className="absolute h-[320.00px] w-[426.67px] flex flex-row justify-start items-center flex-nowrap gap-[50px] left-[0.00px] top-[0.00px]"
                    ></div>
                  </div>
                </div>

                <div
                  id="_236_1284__Frame_1171276975"
                  className="absolute h-[136.00px] w-[1321.00px] left-[-0.50px] flex flex-row justify-start items-start flex-nowrap gap-5 top-[340.00px]"
                >
                  <div
                    id="_236_1285__Frame_1171276994"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1286__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1287__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1288__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1289__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1290__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1291__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1292__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1293__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1294__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1295__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1296__Vector"
                                  src="/assets/blogs/images/vector_27.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1297__Frame_50"
                        >
                          <div
                            id="_236_1298__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1302__Frame_1171276942"
                    className="relative h-[136.00px] w-[427.00px] flex flex-col justify-start items-start flex-nowrap gap-5"
                  >
                    <div
                      id="_236_1303__Frame_122"
                      className="relative h-[70.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[15px] px-2.5 py-0"
                    >
                      <div
                        id="_236_1304__Frame_120"
                        className="relative h-[70.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[5px]"
                      >
                        <span
                          id="_236_1305__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[70.00px] w-full relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[25.0px] font-normal leading-[35.00px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>
                    </div>

                    <div
                      id="_236_1306__Frame_1171276974"
                      className="relative h-[46.00px] w-full"
                    >
                      <div
                        id="_236_1307__Frame_126"
                        className="absolute h-[46.00px] w-[196.50px] left-[calc(50%-98.50px)] flex flex-row justify-start items-center flex-nowrap gap-5 top-[0.00px]"
                      >
                        <div
                          id="_236_1308__Group_3"
                          className="relative h-[46.00px] w-[132.78px]"
                        >
                          <div
                            id="_236_1309__Background"
                            className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[98.68194580078125px] top-[0.00px]"
                          >
                            <div
                              id="_236_1310__Frame_256"
                              className="relative h-[20.00px] flex flex-row justify-start items-center flex-nowrap gap-[5px]"
                            >
                              <span
                                id="_236_1311__Explore"
                                className="flex justify-center text-center items-center h-[20.00px] w-[67.00px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[16.0px] font-medium leading-[24.00px]"
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_236_1312__Img"
                                className="relative h-[19.89px] w-[18.94px]"
                              >
                                <img
                                  id="_236_1313__Vector"
                                  src="/assets/blogs/images/vector_28.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>
                        </div>

                        <div
                          id="_236_1314__Frame_50"
                        >
                          <div
                            id="_236_1315__iconamoon_profile"
                          >
                            <img
                              id="_236_1013__Group"
                              src="/assets/blogs/images/image.png"
                              alt="Group"
                              className="absolute h-[43.72px] w-[43.72px] mt-[-22px]"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_236_1319__Frame_1171276993"
                    className="relative h-[136.00px] w-[427.00px]"
                  ></div>
                </div>
              </div>
            </div>
          </div>

                    <Footer />
        </div>

        <Navbar activePage="blogs" />
      </DesktopPageWrapper>
      {/* ====== MOBILE VIEW ====== */}
      <div className="md:hidden bg-white w-full overflow-x-hidden">
      <div
        id="_264_672__Blog"
        className="relative overflow-hidden bg-white h-[7737px] w-full mx-auto"
      >
        <MobBanner iconSrc="/assets/homemob/images/group_5.svg" />
        <div
          id="_264_673__Frame_1171277067"
          className="absolute h-[7800px] w-[calc(100%-16px)] left-[8.00px] top-[78.00px]"
        >
          <div
            id="_264_674__Frame_1171277068"
            className="absolute h-[7800px] w-full flex flex-col justify-start items-start flex-nowrap gap-[25px] left-[0.00px] top-[0.00px]"
          >
            <div
              id="_264_675__Section"
              className="relative overflow-hidden bg-[rgba(246,247,249,1.00)] h-[495.00px] w-full rounded-[22.8799991607666px]"
            >
              <div className="w-full flex flex-col items-center pt-6">
                <h1
                  id="_264_676__Heading_1___Explore_"
                  className="w-full text-[46.0px] font-normal leading-[54.00px] text-center text-[rgba(17,17,17,1.00)]"
                >
                  Explore Blogs
                </h1>
                <p
                  id="_264_677__We_offer_a_comprehen"
                  className="mt-4 max-w-[320px] text-[16.0px] font-normal leading-[24.00px] text-center text-[rgba(109,109,110,0.80)]"
                >
                  We offer a comprehensive range of services designed to meet
                  the unique needs of your business. From strategy
                  development to risk management, our expert team is dedicated
                  to driving your success.
                </p>
              </div>
              <div
                id="_264_678__Container"
                className="absolute overflow-hidden bg-white h-[46.48%] w-[95.93%] rounded-[15.260000228881836px] left-[50%] -translate-x-[50%] top-[calc(100%_*_0.52)]"
              ></div>
            </div>

            <MobServiceCategories />

            <div
              id="_264_695__Section"
              className="relative overflow-hidden bg-[rgba(246,247,249,1.00)] h-[5620.00px] w-full rounded-[22.8799991607666px]"
            >
              <div
                id="_264_697__Border"
                className="absolute border h-[30.00px] w-[133.00px] left-[calc(50%-67.00px)] rounded-[228.8300018310547px] border-[#202833ff] border-solid top-[38.00px]"
              >
                <span
                  id="_264_698__Case_Studies"
                  className="flex justify-center text-center items-center h-[14.00px] w-[103.39px] absolute left-[calc(50%-51.50px)] top-[calc(50%-7.00px)]"
                >
                  <span
                    className="whitespace-nowrap bg-[rgba(32,40,51,1.00)] bg-clip-text text-transparent not-italic text-[12.0px] font-medium leading-[13.73px] tracking-[1.715999960899353px] uppercase"
                    
                  >
                    Case Studies
                  </span>
                </span>
              </div>

              <span
                id="_264_699__Heading_2___Our_Succ"
                className="flex justify-center text-center items-center h-[79.12px] w-[359.56px] absolute left-[calc(50%-179.32px)] top-[89.97px]"
              >
                <span
                  className="bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[34.0px] font-normal leading-[38.13px]"
                  
                >
                  Our Success Stories Real
                  <br />
                  Results, Real Impact
                </span>
              </span>
              <span
                id="_264_700__Explore_how_we_drive"
                className="flex justify-center text-center items-center h-[64.75px] w-[343.16px] absolute left-[calc(50%-169.33px)] top-[192.09px]"
              >
                <span
                  className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[16.0px] font-normal leading-[22.88px]"
                  
                >
                  Explore how we drive impactful results for our
                  <br />
                  clients. Lorem ipsum dolor sit amet, consectetur
                  <br />
                  adipiscing elit
                </span>
              </span>

              <div
                id="_264_701__Frame_278"
                className="absolute h-[380.00px] w-[calc(100%-16px)] left-[8px] flex flex-col justify-start items-start flex-nowrap gap-[30px] top-[287.00px]"
              >
                <div
                  id="_264_702__Frame_276"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_703__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_704__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_705__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_706__image_26"
                          className="absolute h-[195.00px] w-[157.00px] left-[calc(50%-78.00px)] top-[calc(50%-97.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_26.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_707__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_708__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_709__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_710__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_711__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_712__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_713__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_714__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_715__Vector"
                                  src="/assets/bl/images/vector_10.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_716__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_717__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_718__Group"
                                src="/assets/bl/images/group.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_721__Frame_284"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_722__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_723__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_724__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_725__image_10"
                          className="absolute h-[123.00px] w-[174.00px] left-[calc(50%-87.00px)] top-[calc(50%-61.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_10.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_726__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_727__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_728__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_729__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_730__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_731__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_732__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_733__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_734__Vector"
                                  src="/assets/bl/images/vector_11.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_735__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_736__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_737__Group"
                                src="/assets/bl/images/group_1.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_740__Frame_285"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_741__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_742__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_743__Frame_286"
                        className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                      >
                        <div
                          id="_264_744__Frame_239"
                          className="relative h-[379.00px] w-full"
                        >
                          <div
                            id="_264_745__Listitem___Link"
                            className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                          >
                            <div
                              id="_264_746__Container"
                              className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                            >
                              <div
                                id="_264_747__image_1"
                                className="absolute h-[133.00px] w-[168.00px] left-[calc(50%-84.00px)] top-[calc(50%-66.04px)]"
                                style={{
                                  background:
                                    "url(/assets/bl/images/image_1.png) 100% / cover no-repeat",
                                }}
                              ></div>
                            </div>

                            <div
                              id="_264_748__Frame_273"
                              className="relative h-[57.00px] w-full"
                            >
                              <span
                                id="_264_749__Tuna_Fish_Guide__Qua"
                                className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                              >
                                <span
                                  className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                                  
                                >
                                  Tuna Fish Guide: Quality, Processing, and
                                  Global Demand
                                </span>
                              </span>
                            </div>

                            <div
                              id="_264_750__Frame_1171276943"
                              className="relative h-[40.10px] w-full"
                            >
                              <div
                                id="_264_751__Frame_274"
                                className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                              >
                                <div
                                  id="_264_752__Group_3"
                                  className="relative h-[40.10px] w-[115.74px]"
                                >
                                  <div
                                    id="_264_753__Background"
                                    className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                                  >
                                    <span
                                      id="_264_754__Explore"
                                      className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                                    >
                                      <span
                                        className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                        
                                      >
                                        Explore
                                      </span>
                                    </span>
                                    <div
                                      id="_264_755__Img"
                                      className="relative h-[17.34px] w-[16.51px]"
                                    >
                                      <img
                                        id="_264_756__Vector"
                                        src="/assets/bl/images/vector_12.svg"
                                        alt="Vector"
                                        className="absolute top-[calc(100%_*_0.07)]"
                                      />
                                    </div>
                                  </div>
                                </div>

                                <div
                                  id="_264_757__Frame_50"
                                  className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                                >
                                  <div
                                    id="_264_758__iconamoon_profile"
                                    className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                                  >
                                    <img
                                      id="_264_759__Group"
                                      src="/assets/bl/images/group_2.svg"
                                      alt="Group"
                                      className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                                    />
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>

                      <div
                        id="_264_762__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_763__image-removebg-previ"
                          className="absolute h-[145.00px] w-[148.00px] left-[calc(50%-74.00px)] top-[calc(50%-72.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/imageremovebgpreview_4.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_764__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_765__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_766__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_767__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_768__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_769__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_770__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_771__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_772__Vector"
                                  src="/assets/bl/images/vector_13.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_773__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_774__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_775__Group"
                                src="/assets/bl/images/group_3.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_778__Frame_287"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_779__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_780__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_781__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_782__image_242"
                          className="absolute h-[159.00px] w-[148.00px] left-[calc(50%-74.00px)] top-[calc(50%-79.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_242.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_783__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_783__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_784__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_785__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_786__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_787__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_788__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_789__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_790__Vector"
                                  src="/assets/bl/images/vector_14.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_791__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_792__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_793__Group"
                                src="/assets/bl/images/group_4.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_796__Frame_288"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_797__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_798__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_799__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_800__image_47"
                          className="absolute h-[79.00px] w-[356.00px] left-[calc(50%-178.00px)] top-[calc(50%-39.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_47.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_801__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_802__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_803__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_804__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_805__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_806__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_807__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_808__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_809__Vector"
                                  src="/assets/bl/images/vector_15.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_810__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_811__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_812__Group"
                                src="/assets/bl/images/group_5.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_815__Frame_290"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_816__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_817__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_818__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <img
                          id="_264_820__Vector"
                          src="/assets/bl/images/vector_16.svg"
                          alt="Vector"
                          className="absolute left-[63.00px] top-[87.26px]"
                        />
                        <span
                          id="_264_821__Resora"
                          className="flex justify-start text-left items-start h-[59.00px] w-[139.00px] absolute left-[157.76px] top-[86.00px]"
                        >
                          <span>
                            <span
                              className="whitespace-nowrap bg-[rgba(31,100,255,1.00)] bg-clip-text text-transparent not-italic text-[39.666664123535156px] font-semibold"
                              
                            >
                              R
                            </span>
                            <span
                              className="whitespace-nowrap bg-[rgba(31,100,255,1.00)] bg-clip-text text-transparent not-italic text-[39.666664123535156px] font-medium"
                              
                            >
                              e
                            </span>
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[39.666664123535156px] font-medium"
                              
                            >
                              sora
                            </span>
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_822__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_823__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_824__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_825__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_826__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_827__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_828__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_829__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_830__Vector"
                                  src="/assets/bl/images/vector_17.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_831__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_832__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_833__Group"
                                src="/assets/bl/images/group_6.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_836__Frame_291"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_837__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_838__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_839__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_840__image_63"
                          className="absolute h-[76.17px] w-[271.85px] left-[calc(50%-135.92px)] top-[calc(50%-37.63px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_63.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_841__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_842__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_843__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_844__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_845__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_846__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_847__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_848__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_849__Vector"
                                  src="/assets/bl/images/vector_18.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_850__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_851__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_852__Group"
                                src="/assets/bl/images/group_7.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_855__Frame_289"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_856__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_857__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_858__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <img
                          id="_264_859__Group_1000008334"
                          src="/assets/bl/images/group_1000008334.svg"
                          alt="Group_1000008334"
                          className="absolute left-[calc(100%_*_0.35)] top-[calc(100%_*_0.19)]"
                        />
                      </div>

                      <div
                        id="_264_862__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_863__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_864__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_865__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_866__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_867__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_868__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_869__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_870__Vector"
                                  src="/assets/bl/images/vector_19.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_871__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_872__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_873__Group"
                                src="/assets/bl/images/group_8.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_876__Frame_292"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_877__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_878__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_879__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_880__image_247"
                          className="absolute h-[84.00px] w-[185.00px] left-[calc(50%-93.00px)] top-[calc(50%-42.04px)] rounded-[89px]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_247.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_881__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_882__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_883__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_884__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_885__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_886__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_887__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_888__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_889__Vector"
                                  src="/assets/bl/images/vector_20.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_890__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_891__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_892__Group"
                                src="/assets/bl/images/group_9.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_895__Frame_293"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_896__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_897__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_898__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_899__image_15"
                          className="absolute h-[90.00px] w-[243.00px] left-[calc(50%-121.00px)] top-[calc(50%-45.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_15.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_900__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_901__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_902__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_903__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_904__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_905__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_906__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_907__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_908__Vector"
                                  src="/assets/bl/images/vector_21.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_909__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_910__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_911__Group"
                                src="/assets/bl/images/group_10.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_914__Frame_294"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_915__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_916__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_917__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_918__Screenshot_2025-12-1"
                          className="absolute h-[89.00px] w-[219.00px] left-[calc(50%-109.00px)] top-[calc(50%-44.04px)] rounded-[45px]"
                          style={{
                            background:
                              "url(/assets/bl/images/screenshot_20251219_at_103139_pm_1.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_919__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_920__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_921__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_922__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_923__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_924__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_925__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_926__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_927__Vector"
                                  src="/assets/bl/images/vector_22.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_928__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_929__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_930__Group"
                                src="/assets/bl/images/group_11.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_933__Frame_295"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_934__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_935__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_936__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_937__image_248"
                          className="absolute h-[152.00px] w-[159.79px] left-[calc(50%-79.40px)] top-[calc(50%-76.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/image_248.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_938__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_939__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_940__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_941__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_942__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_943__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_944__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_945__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_946__Vector"
                                  src="/assets/bl/images/vector_23.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_947__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_948__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_949__Group"
                                src="/assets/bl/images/group_12.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_264_952__Frame_296"
                  className="relative h-[379.00px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                >
                  <div
                    id="_264_953__Frame_239"
                    className="relative h-[379.00px] w-full"
                  >
                    <div
                      id="_264_954__Listitem___Link"
                      className="absolute h-[374.00px] w-[calc(100%+-0.00px)] flex flex-col justify-start items-start flex-nowrap gap-[25px] px-0 py-[3px] rounded-[22.8799991607666px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_264_955__Container"
                        className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
                      >
                        <div
                          id="_264_956__image_248"
                          className="absolute h-[152.00px] w-[159.79px] left-[calc(50%-79.40px)] top-[calc(50%-76.04px)]"
                          style={{
                            background:
                              "url(/assets/bl/images/imageremovebgpreview_4.png) 100% / cover no-repeat",
                          }}
                        ></div>
                      </div>

                      <div
                        id="_264_957__Frame_273"
                        className="relative h-[57.00px] w-full"
                      >
                        <span
                          id="_264_958__Tuna_Fish_Guide__Qua"
                          className="flex justify-center text-center items-center h-[57.00px] w-[353.00px] absolute top-[-0.08px] left-[7.00px]"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.80)] bg-clip-text text-transparent not-italic text-[24.0px] font-normal leading-[30.50px]"
                            
                          >
                            Tuna Fish Guide: Quality, Processing, and Global
                            Demand
                          </span>
                        </span>
                      </div>

                      <div
                        id="_264_959__Frame_1171276943"
                        className="relative h-[40.10px] w-full"
                      >
                        <div
                          id="_264_960__Frame_274"
                          className="absolute h-[40.10px] w-[169.00px] left-[calc(50%-88.00px)] top-[-0.08px] flex flex-row justify-start items-center flex-nowrap gap-[15px] pl-[7px] pr-0 py-0"
                        >
                          <div
                            id="_264_961__Group_3"
                            className="relative h-[40.10px] w-[115.74px]"
                          >
                            <div
                              id="_264_962__Background"
                              className="absolute bg-[rgba(1,97,254,1.00)] h-[46px] w-[132px] left-[calc(50%-66px)] flex flex-row justify-center items-center flex-nowrap gap-[8px] rounded-[86.01754760742188px] top-[0.00px]"
                            >
                              <span
                                id="_264_963__Explore"
                                className="flex justify-center text-center items-center h-[17.43px] w-[58.40px] relative"
                              >
                                <span
                                  className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.775653839111328px] font-medium leading-[20.65px] tracking-[-0.06845570355653763px]"
                                  
                                >
                                  Explore
                                </span>
                              </span>
                              <div
                                id="_264_964__Img"
                                className="relative h-[17.34px] w-[16.51px]"
                              >
                                <img
                                  id="_264_965__Vector"
                                  src="/assets/bl/images/vector_24.svg"
                                  alt="Vector"
                                  className="absolute top-[calc(100%_*_0.07)]"
                                />
                              </div>
                            </div>
                          </div>

                          <div
                            id="_264_966__Frame_50"
                            className="relative bg-[rgba(1,97,254,1.00)] h-[22.11px] w-[22.11px] flex flex-row justify-start items-center flex-nowrap gap-2 p-2 rounded-[49.684879302978516px]"
                          >
                            <div
                              id="_264_967__iconamoon_profile"
                              className="relative overflow-hidden h-[20.92px] w-[20.92px]"
                            >
                              <img
                                id="_264_968__Group"
                                src="/assets/bl/images/group_13.svg"
                                alt="Group"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <MobFooter />
            </div>
          </div>
        </div>

      </div>

    </>
  );
};
export default BlogDesktop;

