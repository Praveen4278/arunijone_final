import React from "react";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import { useNavigate } from "react-router-dom";
import DesktopPageWrapper from "../components/DesktopPageWrapper";
import MobBanner from "../components/MobBanner";
import PortfolioMetadata from "../components/PortfolioMetadata";
import MobFooter from "../components/MobFooter";

const FAQDetailsDesktop = () => {

  const navigate = useNavigate();
  return (
    <>
      <DesktopPageWrapper className="min-h-screen">
        <Navbar activePage="home" />

        <div
          id="_240_245__Frame_1171277074"
          className="relative w-[1420.00px] flex flex-col justify-start items-start flex-nowrap gap-[50px] left-[10.00px] top-[107.00px] min-h-[5080.28px]"
        >
          <div
            id="_231_4684__Section"
            className="relative overflow-hidden bg-[#F6F7F9] min-h-[926.00px] w-full rounded-[20.41314125061035px]"
          >
            {/* Heading */}
            <span
              id="_231_4685__Heading_1___Operatio"
              className="absolute flex justify-center text-center items-center h-[49.00px] w-[1329.00px] left-[calc(50%-664.00px)] top-[45.00px]"
            >
              <span className="whitespace-nowrap text-[#111111] text-[40.811283111572266px] font-normal leading-[47px] not-italic">
                My Rameswaram Trip – Travel &amp; Temple Service Platform
              </span>
            </span>

            {/* Metadata Row — Frame 233 */}
            <PortfolioMetadata />

            {/* Image Box */}
            <div
              id="_231_4687__Image_Box"
              className="absolute overflow-hidden h-[628.00px] w-[1331.00px] left-[calc(50%-665.00px)] top-[27.65%] rounded-[20.41314125061035px]"
            >
              <div
                id="_231_4688__67bdaa31ce4ff0b778ed"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(0,0,0,0.20)0%,rgba(0,0,0,0.20)100%)] h-[627.50px] w-[1335.11px] left-[0.00px] top-[0.00px]"
                style={{ filter: "blur(0.0px)" }}
              >
                <div
                  id="_231_4689__Gemini_Logo_Removed_"
                  className="absolute h-[628.00px] w-full left-[0.00px] top-[0.00px]"
                  style={{
                    background:
                      "url(/assets/portfolio/images/gemini_logo_removed_1.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>
          </div>

          <div
            id="_240_295__Frame_1171276955"
            className="relative w-[94%] flex flex-col justify-start items-start flex-nowrap gap-[30px] min-h-[3042.00px]"
          >
            <div
              id="_240_296__Frame_1171276954"
              className="relative overflow-hidden bg-[rgba(246,247,249,1.00)] w-full flex flex-col justify-start items-center flex-nowrap gap-2.5 px-[42px] py-[50px] rounded-[23px] min-h-[3042.00px]"
            >
              <div
                id="_240_297__Group_37"
                className="relative h-[691.00px] w-full"
              >
                <div
                  id="_240_298__Frame_196"
                  className="absolute h-[691.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-10 left-[0.00px] top-[0.00px]"
                >
                  <div
                    id="_240_299__Frame_195"
                    className="relative w-[calc(100%-0px-0px)] flex flex-col justify-start items-start flex-nowrap gap-[30px]"
                  >
                    <div
                      id="_240_300__Border"
                      className="relative border h-[36.00px] w-[138.00px] rounded-[272.07000732421875px] border-[#202833ff] border-solid"
                    >
                      <span
                        id="_240_301__Project_1"
                        className="flex justify-start text-left items-center h-[14.00px] w-[91.00px] absolute left-[calc(50%-45.00px)] top-[calc(50%-7.00px)]"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(32,40,51,1.00)] bg-clip-text text-transparent not-italic text-[14.0px] font-medium leading-[22.54px] tracking-[2.38100004196167px] uppercase"
                        >
                          Project 1
                        </span>
                      </span>
                    </div>

                    <div
                      id="_240_302__Frame_194"
                      className="relative h-[91.00px] w-[calc(100%-0px-0px)] flex flex-row justify-start items-center flex-nowrap gap-[432px]"
                    >
                      <div
                        id="_240_303__Frame_193"
                        className="relative h-[71.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                      >
                        <span
                          id="_240_304__Heading_2___Explore_"
                          className="flex justify-start text-left items-center h-[90.00px] w-[770.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[42.0px] font-normal leading-[45.00px]"
                          >
                            My Rameswaram Trip – Travel &amp; Temple Service
                            Platform
                          </span>
                        </span>
                      </div>

                      <div
                        id="_240_305__Link"
                        className="relative bg-[rgba(1,97,254,1.00)] h-[40.00px] flex flex-row justify-start items-center flex-nowrap gap-[7px] px-5 py-[3px] rounded-[85.04351043701172px]"
                      >
                        <span
                          id="_240_306__View_All"
                          className="flex justify-center text-center items-center h-[14.25px] w-[45.78px] relative"
                        >
                          <span
                            className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[12.003317832946777px] font-medium leading-[17.96px] tracking-[-0.06001658737659454px]"
                          >
                            View All
                          </span>
                        </span>
                        <div
                          id="_240_307__67a22e78f02af13d518a"
                          className="relative overflow-hidden h-[20.41px] w-[20.41px]"
                        >
                          <div
                            id="_240_308__67a22e78f02af13d518a"
                            className="absolute overflow-hidden h-[20.41px] w-[20.41px] left-[-0.00px] top-[-0.00px]"
                          >
                            <div
                              id="_240_309__67a22e78f02af13d518a"
                              className="absolute overflow-hidden h-[20.41px] w-[20.41px] left-[calc(50%-10.20px)] top-[calc(50%-10.20px)]"
                            >
                              <img
                                id="_240_310__Vector"
                                src="/assets/FAQdetails/images/vector_2.svg"
                                alt="Vector"
                                className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.25)]"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div id="_240_311__Group_36" className="absolute h-[3004px] w-full left-[0px] top-[209px]">
                    <div
                      id="_240_312__Frame_1171277056"
                      className="absolute h-[2687.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-[40px] left-[0.00px] top-[258.00px]"
                    >
                      <div
                        id="_240_313__Paragraph_Background"
                        className="relative bg-white h-[56.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[461px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_314__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_315__UX_Research_Approach"
                            className="flex justify-start text-left items-center h-[37.00px] w-[266.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              UX Research Approach
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_316__Frame_188"
                          className="relative h-[30.00px] w-[539.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_317__What_UX_research_met"
                            className="flex justify-start text-left items-center h-[30.00px] w-[532.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What UX research methods were used in this
                              project?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_318__Paragraph_Background"
                        className="relative bg-white h-[56.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_319__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_320__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_321__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_322__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_323__Paragraph_Background"
                        className="relative bg-white h-[56.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_324__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_325__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_326__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_327__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_328__Paragraph_Background"
                        className="relative bg-white h-[60.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_329__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_330__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_331__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_332__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_333__Paragraph_Background"
                        className="relative bg-white h-[56.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_334__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_335__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_336__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_337__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_338__Paragraph_Background"
                        className="relative bg-white h-[56.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_339__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_340__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_341__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_342__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_343__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_344__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_345__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_346__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_347__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_348__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_349__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_350__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_351__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_352__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_353__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_354__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_355__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_356__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_357__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_358__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_359__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_360__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_361__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_362__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_363__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_364__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_365__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_366__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_367__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_368__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_369__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_370__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_371__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_372__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_373__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_374__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_375__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_376__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_377__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_378__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_379__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_380__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_381__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_382__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_383__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_384__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_385__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_386__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_387__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_388__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_389__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_390__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_391__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_392__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_393__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_394__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_395__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_396__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_397__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_398__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_399__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_400__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_401__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_402__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_403__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_404__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_405__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_406__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_407__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_408__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_409__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_410__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_411__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_412__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_413__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_414__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_415__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_416__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_417__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_418__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_419__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_420__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_421__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_422__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>

                      <div
                        id="_240_423__Paragraph_Background"
                        className="relative bg-white h-[57.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[446px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_424__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_425__User_Problems_Identi"
                            className="flex justify-start text-left items-center h-[37.00px] w-[278.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-black bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              User Problems Identified
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_426__Frame_188"
                          className="relative h-[30.00px] w-[527.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_427__What_key_user_proble"
                            className="flex justify-start text-left items-center h-[30.00px] w-[517.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                            >
                              What key user problems did the research uncover?
                            </span>
                          </span>
                        </div>
                      </div>
                    </div>

                    <div
                      id="_240_428__Frame_1171277007"
                      className="absolute h-[60.00px] w-full flex flex-col justify-start items-start flex-nowrap gap-2.5 left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_240_429__Paragraph_Background"
                        className="relative bg-[rgba(1,97,254,1.00)] h-[69.00px] w-full flex flex-row justify-start items-center flex-nowrap gap-[628px] pl-[15px] py-1.5 rounded-[20px]"
                      >
                        <div
                          id="_240_430__Frame_189"
                          className="relative h-[37.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <span
                            id="_240_431__Overview"
                            className="flex justify-start text-left items-center h-[37.00px] w-[108.00px] relative"
                          >
                            <span
                              className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[27.200000762939453px] font-normal leading-[36.27px] capitalize"
                            >
                              Overview
                            </span>
                          </span>
                        </div>

                        <div
                          id="_240_432__Frame_188"
                          className="relative h-[30.00px] w-[437.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5"
                        >
                          <div
                            id="_240_433__Frame_1171277006"
                            className="relative h-[30.00px] flex flex-row justify-center items-center flex-nowrap gap-2.5"
                          >
                            <span
                              id="_240_434__What_is_the_My_Rames"
                              className="flex justify-start text-left items-center h-[30.00px] w-[446.00px] relative"
                            >
                              <span
                                className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[30.00px]"
                              >
                                What is the My Rameswaram Trip UX/UI project
                                about?
                              </span>
                            </span>
                          </div>
                        </div>
                      </div>

                      <div
                        id="_240_435__Background"
                        className="absolute bg-white h-[128.00px] w-[1315.00px] left-[calc(50%-657.50px)] flex flex-row justify-start items-center flex-nowrap gap-6 rounded-[20px] top-[89.00px]"
                      >
                        <span
                          id="_240_436__This_project_focuses"
                          className="flex justify-start text-left items-center h-[84.00px] w-[1270.00px] absolute left-[22.00px] top-[22.00px]"
                        >
                          <span
                            className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[20.0px] font-normal leading-[28.00px] capitalize"
                          >
                            This project focuses on designing a user-centered
                            digital platform that combines pilgrimage services
                            and travel planning for Rameswaram. The goal is to
                            create a seamless experience for devotees and
                            travelers by simplifying temple services, travel
                            bookings, and local guidance through an intuitive
                            and respectful interface.
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div
            id="_240_437__Section"
            className="relative overflow-hidden h-[600.00px] w-full rounded-[22.518083572387695px]"
          >
            <span
              id="_240_438__Heading_1___About_ou"
              className="flex justify-start text-left items-center h-[54.00px] w-[310.00px] absolute left-[47.00px] top-[51.00px]"
            >
              <span
                className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[46.0px] font-normal leading-[51.65px]"
              >
                Our Products
              </span>
            </span>
            <span
              id="_240_439__At_Arunijone__I_take"
              className="flex justify-start text-left items-center h-[64.00px] w-[723.00px] absolute left-[648.00px] top-[51.00px]"
            >
              <span
                className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[16.0px] font-normal leading-[22.00px]"
              >
                At Arunijone, I take pride in delivering diverse, high-impact
                projects across multiple domains. With experience in UX/UI
                design, digital marketing, stock research, aquascaping, 3D
                design, and coir product solutions, every project reflects
                innovation, precision, and measurable results
              </span>
            </span>
            <div
              id="_240_440__Container"
              className="absolute overflow-hidden h-[370.39px] w-[351.33px] left-[-124.94px] rounded-[22.518083572387695px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] top-[177.92px]"
            >
              <div
                id="_240_441__67b5af0aadbf9fd23486"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%] top-[calc(100%_*_0.00)]"
              >
                <div
                  id="_240_442__image_26"
                  className="absolute h-[249.00px] w-[202.00px] left-[calc(50%-100.82px)] top-[calc(50%-124.22px)]"
                  style={{
                    background:
                      "url(/assets/Home/images/image_26.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>

            <div
              id="_240_443__Container"
              className="absolute overflow-hidden h-[370.39px] w-[351.33px] rounded-[22.518083572387695px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] left-[241.59px] top-[177.92px]"
            >
              <div
                id="_240_444__67b5af0b7a74e5519cee"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%] top-[calc(100%_*_0.00)]"
              >
                <div
                  id="_240_445__image_10"
                  className="absolute h-[138.00px] w-[196.00px] left-[calc(50%-98.35px)] top-[calc(50%-69.22px)]"
                  style={{
                    background:
                      "url(/assets/FAQdetails/images/image_10.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>

            <div
              id="_240_446__Container"
              className="absolute overflow-hidden h-[370.39px] w-[351.33px] rounded-[22.518083572387695px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] left-[608.12px] top-[177.92px]"
            >
              <div
                id="_240_447__67b5af0cadbf9fd23486"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%]"
              >
                <div
                  id="_240_448__image_1"
                  className="absolute h-[185.00px] w-[233.00px] left-[calc(50%-116.38px)] top-[calc(50%-92.72px)]"
                  style={{
                    background:
                      "url(/assets/FAQdetails/images/image_1.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>

            <div
              id="_240_449__Container"
              className="absolute overflow-hidden h-[370.39px] w-[351.33px] rounded-[22.518083572387695px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] left-[974.65px] top-[177.92px]"
            >
              <div
                id="_240_450__67b5af0a9616f526f030"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%]"
              >
                <div
                  id="_240_451__image_47"
                  className="absolute h-[81.00px] w-[366.00px] left-[calc(50%-183.92px)] top-[calc(50%-41.22px)]"
                  style={{
                    background:
                      "url(/assets/FAQdetails/images/image_47.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>

            <div
              id="_240_452__Container"
              className="absolute overflow-hidden h-[370.39px] w-[351.33px] rounded-[22.518083572387695px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] left-[1341.19px] top-[177.92px]"
            >
              <div
                id="_240_453__67b5af0aadbf9fd23486"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%]"
              >
                <div
                  id="_240_454__image_63"
                  className="absolute h-[74.00px] w-[264.00px] left-[calc(50%-131.95px)] top-[calc(50%-37.22px)]"
                  style={{
                    background:
                      "url(/assets/FAQdetails/images/image_63.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>
          </div>

          <Footer />
        </div>
      </DesktopPageWrapper>




      {/* ====== MOBILE VIEW ====== */}
      <div className="md:hidden bg-white w-full overflow-x-hidden">
        <div
          id="_266_679__Blog_Details"
          className="relative overflow-hidden bg-white w-full mx-auto min-h-[4921px]"
        >
          <MobBanner iconSrc="/assets/faq-detailsmob/images/group.svg" />

          <div
            id="_266_756__Section"
            className="absolute overflow-hidden bg-[rgba(246,247,249,1.00)] h-[949.00px] w-[calc(93%)] left-[8.00px] flex flex-col justify-start items-center flex-nowrap pt-[25px] pb-[7px] px-[7px] rounded-[calc(22.8799991607666px_*_1.0)] top-[78.00px]"
          >
            <div
              id="_266_757__Container"
              className="relative w-full flex flex-col justify-start items-center flex-nowrap gap-[30px]"
            >
              <div
                id="_266_758__Container"
                className="relative h-[687.80px] w-[calc(100%-0px)] flex flex-col justify-start items-center flex-nowrap gap-[30px]"
              >
                <div
                  id="_266_759__Container"
                  className="relative h-[188.73px] flex flex-col justify-start items-center flex-nowrap gap-[21px]"
                >
                  <div
                    id="_266_760__Heading_1"
                    className="relative h-[188.73px] w-full flex flex-col justify-start items-center flex-nowrap"
                  >
                    <span
                      id="_266_761__My_Rameswaram_Trip__"
                      className="flex justify-center text-center items-center h-[188.00px] w-full max-w-[359px] mx-auto relative"
                    >
                      <span
                        className="bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[40.811283111572266px] font-normal leading-[46.82px]"

                      >
                        My Rameswaram Trip – Travel &amp; Temple Service Platform
                      </span>
                    </span>
                  </div>
                </div>

                <div
                  id="_266_762__Container"
                  className="relative h-[469.07px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap gap-5"
                >
                  <div
                    id="_266_763__Container"
                    className="relative h-[57.02px] flex flex-col justify-start items-start flex-nowrap gap-2"
                  >
                    <div
                      id="_266_764__Container"
                      className="relative h-[23.88px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_765__Name_Corp"
                        className="flex justify-center text-center items-center h-[23.00px] w-[76.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[22.88px]"

                        >
                          Name Corp
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_766__Container"
                      className="relative h-[24.69px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_767__Avla_Nettons_Exports"
                        className="flex justify-center text-center items-center h-[24.00px] w-[158.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[23.81px]"

                        >
                          Avla Nettons Exports
                        </span>
                      </span>
                    </div>
                  </div>

                  <div
                    id="_266_768__Container"
                    className="relative h-[60.01px] flex flex-col justify-start items-start flex-nowrap gap-2"
                  >
                    <div
                      id="_266_769__Container"
                      className="relative h-[23.88px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_770__Project_Duration"
                        className="flex justify-center text-center items-center h-[23.00px] w-[105.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[22.88px]"

                        >
                          Project Duration
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_771__Container"
                      className="relative h-[27.69px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_772__3_Months"
                        className="flex justify-center text-center items-center h-[27.00px] w-[72.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[26.70px]"

                        >
                          3 Months
                        </span>
                      </span>
                    </div>
                  </div>

                  <div
                    id="_266_773__Container"
                    className="relative h-[60.01px] flex flex-col justify-start items-start flex-nowrap gap-2"
                  >
                    <div
                      id="_266_774__Container"
                      className="relative h-[23.88px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_775__Release_Date"
                        className="flex justify-center text-center items-center h-[23.00px] w-[86.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[22.88px]"

                        >
                          Release Date
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_776__Container"
                      className="relative h-[27.69px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_777__Jan_1__2025"
                        className="flex justify-center text-center items-center h-[27.00px] w-[87.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[26.70px]"

                        >
                          Jan 1, 2025
                        </span>
                      </span>
                    </div>
                  </div>

                  <div
                    id="_266_778__Container"
                    className="relative h-[60.02px] flex flex-col justify-start items-start flex-nowrap gap-2"
                  >
                    <div
                      id="_266_779__Container"
                      className="relative h-[23.88px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_780__Read_Time"
                        className="flex justify-center text-center items-center h-[23.00px] w-[70.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[22.88px]"

                        >
                          Read Time
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_781__Container"
                      className="relative h-[27.69px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_782__3_Min"
                        className="flex justify-center text-center items-center h-[27.00px] w-[43.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[26.70px]"

                        >
                          3 Min
                        </span>
                      </span>
                    </div>
                  </div>

                  <div
                    id="_266_783__Container"
                    className="relative h-[60.01px] flex flex-col justify-start items-start flex-nowrap gap-2"
                  >
                    <div
                      id="_266_784__Container"
                      className="relative h-[23.88px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_785__Site_View"
                        className="flex justify-center text-center items-center h-[23.00px] w-[60.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[22.88px]"

                        >
                          Site View
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_786__Container"
                      className="relative h-[27.69px] w-[calc(100%-0px-0px)] flex flex-col justify-start items-center flex-nowrap"
                    >
                      <span
                        id="_266_787__myrameswaramtrip_com"
                        className="flex justify-center text-center items-center h-[27.00px] w-[181.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[18.0px] font-normal leading-[26.70px]"

                        >
                          myrameswaramtrip.com
                        </span>
                      </span>
                    </div>
                  </div>

                  <div
                    id="_266_788__Frame_225"
                    className="relative h-[72.00px] w-[184.00px] flex flex-col justify-start items-start flex-nowrap gap-3"
                  >
                    <div
                      id="_266_789__Frame_225"
                      className="relative w-[calc(100%-0px-0px)] flex flex-row justify-center items-center flex-nowrap gap-2.5"
                    >
                      <span
                        id="_266_790__Work_By"
                        className="flex justify-center text-center items-center h-[16.50px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[16.0px] font-light leading-[20.41px]"

                        >
                          Work By
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_791__Frame_233"
                      className="absolute h-[43.72px] w-[97.44px] left-[calc(50%-48.50px)] flex flex-row justify-start items-center flex-nowrap gap-2.5 top-[28.50px]"
                    >
                      <div
                        id="_266_792__Frame_49"
                        className="relative h-[23.72px] w-[23.72px] flex flex-row justify-start items-center flex-nowrap gap-2.5 p-2.5"
                      >
                        <div
                          id="_266_793__Frame_108"
                          className="absolute h-[43.72px] w-[43.72px] left-[0.00px] top-[0.00px]"
                        >
                          <img
                            id="_266_794__Vector"
                            src="/assets/faq-detailsmob/images/vector_9.svg"
                            alt="Vector"
                            className="absolute"
                          />
                          <div
                            id="_266_795__iconamoon_profile"
                            className="absolute overflow-hidden h-[24.00px] w-[24.00px] left-[10.00px] top-[9.86px]"
                          >
                            <img
                              id="_266_796__Group"
                              src="/assets/faq-detailsmob/images/group_1.svg"
                              alt="Group"
                              className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                            />
                          </div>
                        </div>
                      </div>

                      <div
                        id="_266_799__Frame_109"
                        className="relative h-[43.72px] w-[43.72px]"
                      >
                        <img
                          id="_266_800__Vector"
                          src="/assets/faq-detailsmob/images/vector_10.svg"
                          alt="Vector"
                          className="absolute"
                        />
                        <div
                          id="_266_801__iconamoon_profile"
                          className="absolute overflow-hidden h-[24.00px] w-[24.00px] left-[10.00px] top-[9.86px]"
                        >
                          <img
                            id="_266_802__Group"
                            src="/assets/faq-detailsmob/images/group_2.svg"
                            alt="Group"
                            className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.17)]"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_266_805__Container"
                className="relative overflow-hidden bg-white h-[230.08px] w-full rounded-[15.260000228881836px]"
              >
                <div
                  id="_266_806__Gemini_Logo_Removed_"
                  className="absolute h-[230.00px] w-full left-[0px] top-[calc(50%-114.84px)]"
                  style={{
                    background:
                      "url(/assets/faq-detailsmob/images/gemini_logo_removed_1.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>
          </div>

          <div
            id="_266_807__Frame_1171276956"
            className="absolute w-[calc(100%-16px)] left-[8.00px] flex flex-col justify-start items-center flex-nowrap gap-[30px] top-[1109.00px] min-h-[2773.00px]"
          >
            <div
              id="_266_808__Frame_1171276959"
              className="relative bg-[rgba(246,247,249,1.00)] w-full rounded-[23px] min-h-[2773.00px]"
            >
              <div
                id="_266_810__Frame_270"
                className="absolute w-[calc(100%-12px)] flex flex-col justify-start items-center flex-nowrap gap-[25px] left-[6px] top-[35.47px] min-h-[2645.00px]"
              >
                <div
                  id="_266_811__Border"
                  className="relative border h-[30.00px] w-[108.00px] rounded-[228.8300018310547px] border-[#202833ff] border-solid"
                >
                  <span
                    id="_266_812__project_1"
                    className="flex justify-center text-center items-center h-[14.00px] w-[77.00px] absolute left-[calc(50%-38.00px)] top-[calc(50%-7.13px)]"
                  >
                    <span
                      className="whitespace-nowrap bg-[rgba(32,40,51,1.00)] bg-clip-text text-transparent not-italic text-[12.0px] font-medium leading-[13.73px] tracking-[1.715999960899353px] uppercase"

                    >
                      project 1
                    </span>
                  </span>
                </div>

                <div
                  id="_266_813__Frame_1171277004"
                  className="relative h-[622.00px] w-full"
                >
                  <div
                    id="_266_814__Frame_1171277003"
                    className="absolute h-[176.00px] w-[300.00px] left-[32.00px] top-[0.53px]"
                  >
                    <span
                      id="_266_815__Heading_2___Our_Succ"
                      className="flex justify-center text-center items-center h-[176.00px] w-[300.00px] absolute left-[0.00px] top-[0.47px]"
                    >
                      <span
                        className="bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[40.0px] font-normal leading-[46.00px]"

                      >
                        My Rameswaram Trip – Travel &amp; Temple Service Platform
                      </span>
                    </span>
                  </div>

                  <div
                    id="_266_817__Background"
                    className="absolute bg-white h-[209.00px] w-[360.00px] left-[50%] -translate-x-[50%] flex flex-row justify-start items-center flex-nowrap gap-6 p-2.5 rounded-[15px] top-[390.53px]"
                  >
                    <span
                      id="_266_818__This_project_focuses"
                      className="flex justify-center text-center items-center h-[211.00px] w-full relative"
                    >
                      <span
                        className="bg-[rgba(109,109,110,1.00)] bg-clip-text text-transparent not-italic text-[15.0px] font-normal leading-[28.00px] capitalize"

                      >
                        This project focuses on designing a user-centered digital
                        platform that combines pilgrimage services and travel
                        planning for Rameswaram. The goal is to create a seamless
                        experience for devotees and travelers by simplifying
                        temple services, travel bookings, and local guidance
                        through an intuitive and respectful interface.
                      </span>
                    </span>
                  </div>

                  <div
                    id="_266_819__Paragraph_Background"
                    className="absolute bg-[rgba(1,97,254,1.00)] h-[149.00px] w-full rounded-[15px] left-[0.00px] top-[226.53px]"
                  >
                    <div
                      id="_266_820__Frame_189"
                      className="absolute h-[37.00px] w-[111.00px] left-[calc(50%-65.50px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.47px]"
                    >
                      <span
                        id="_266_821__Overview"
                        className="flex justify-start text-left items-center h-[37.00px] w-[111.00px] relative"
                      >
                        <span
                          className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                        >
                          Overview
                        </span>
                      </span>
                    </div>

                    <div
                      id="_266_822__Frame_188"
                      className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[65.47px]"
                    >
                      <span
                        id="_266_823__What_is_the_My_Rames"
                        className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                      >
                        <span
                          className="bg-white bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                        >
                          What is the My Rameswaram Trip UX/UI project about?
                        </span>
                      </span>
                    </div>
                  </div>
                </div>

                <div
                  id="_266_824__Frame_1171277057"
                  className="relative h-[1857.00px] flex flex-col justify-start items-center flex-nowrap gap-[25px]"
                >
                  <div
                    id="_266_825__Group_33"
                    className="relative h-[185.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_826__Paragraph_Background"
                      className="absolute bg-white h-[185.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_827__Frame_189"
                        className="absolute h-[73.00px] w-[187.00px] left-[calc(50%-103.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.47px]"
                      >
                        <span
                          id="_266_828__UX_Research_Approach"
                          className="flex justify-center text-center items-center h-[73.00px] w-[187.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            UX Research Approach
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_829__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.47px]"
                      >
                        <span
                          id="_266_830__What_UX_research_met"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What UX research methods were used in this project?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_831__Group_34"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_832__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_833__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_834__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_835__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_836__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_837__Group_35"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_838__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_839__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_840__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_841__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_842__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_843__Group_36"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_844__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_845__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_846__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_847__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_848__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_849__Group_37"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_850__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_851__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_852__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_853__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_854__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_855__Group_38"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_856__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_857__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_858__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_859__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_860__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_861__Group_39"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_862__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_863__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_864__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_865__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_866__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_867__Group_40"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_868__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_869__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_870__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_871__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_872__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>

                  <div
                    id="_266_873__Group_41"
                    className="relative h-[184.00px] w-[364.00px]"
                  >
                    <div
                      id="_266_874__Paragraph_Background"
                      className="absolute bg-white h-[184.00px] w-[364.00px] rounded-[15px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_875__Frame_189"
                        className="absolute h-[73.00px] w-[247.00px] left-[calc(50%-133.00px)] flex flex-row justify-center items-start flex-nowrap gap-2.5 p-2.5 top-[8.00px]"
                      >
                        <span
                          id="_266_876__User_Problems_Identi"
                          className="flex justify-center text-center items-center h-[73.00px] w-[247.00px] relative"
                        >
                          <span
                            className="bg-black bg-clip-text text-transparent not-italic text-[28.0px] font-normal leading-[36.27px] capitalize"

                          >
                            User Problems Identified
                          </span>
                        </span>
                      </div>

                      <div
                        id="_266_877__Frame_188"
                        className="absolute h-[55.00px] w-[322.00px] left-[calc(50%-171.00px)] flex flex-row justify-center items-center flex-nowrap gap-2.5 p-2.5 top-[101.00px]"
                      >
                        <span
                          id="_266_878__What_key_user_proble"
                          className="flex justify-center text-center items-center h-[55.00px] w-[322.00px] relative"
                        >
                          <span
                            className="bg-[rgba(17,17,17,0.60)] bg-clip-text text-transparent not-italic text-[18.100000381469727px] font-light leading-[27.21px]"

                          >
                            What key user problems did the research uncover?
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>
                </div>

                <div
                  id="_266_879__Link"
                  className="relative border h-[51.00px] flex flex-row justify-start items-center flex-nowrap gap-2 px-[35px] py-[3px] rounded-[109.07755279541016px] border-[#a7a7a7ff] border-solid"
                >
                  <span
                    id="_266_880__Load_More"
                    className="flex justify-center text-center items-center h-[19.00px] w-[79.00px] relative"
                  >
                    <span
                      className="whitespace-nowrap bg-[rgba(68,68,68,1.00)] bg-clip-text text-transparent not-italic text-[15.395560264587402px] font-medium leading-[23.04px] tracking-[-0.07697779685258865px]"

                    >
                      Load More
                    </span>
                  </span>
                </div>
              </div>
            </div>
          </div>

          <div
            id="_266_881__Section"
            className="absolute overflow-hidden h-[661.00px] w-[calc(100%-16px)] left-[8.00px] rounded-[22.8799991607666px] top-[3932.00px]"
          >
            <span
              id="_266_882__Heading_1___About_ou"
              className="flex justify-center text-center items-center h-[55.00px] w-[246.00px] absolute left-[calc(50%-123.50px)] top-[36.00px]"
            >
              <span
                className="whitespace-nowrap bg-[rgba(17,17,17,1.00)] bg-clip-text text-transparent not-italic text-[45.0px] font-normal leading-[52.49px]"

              >
                Our Projects
              </span>
            </span>
            <span
              id="_266_883__At_Arunijone__I_take"
              className="flex justify-center text-center items-center h-[154.00px] w-[335.00px] absolute left-[calc(50%-167.50px)] top-[111.00px]"
            >
              <span
                className="bg-[rgba(109,109,110,0.80)] bg-clip-text text-transparent not-italic text-[16.0px] font-normal leading-[22.00px]"

              >
                At Arunijone, I take pride in delivering diverse, high-impact
                projects across multiple domains. With experience in UX/UI design,
                digital marketing, stock research, aquascaping, 3D design, and
                coir product solutions, every project reflects innovation,
                precision, and measurable results
              </span>
            </span>
            <div
              id="_266_884__Container"
              className="absolute overflow-hidden h-[289.64px] w-[274.39px] left-[-56.36px] rounded-[15.260000228881836px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] top-[361.00px]"
            >
              <div
                id="_266_885__67b5af0aadbf9fd23486"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(254,254,254,1.00)0%,rgba(254,254,254,1.00)100%)] h-[100.00%] w-[100.00%]"
              >
                <div
                  id="_266_886__image_26"
                  className="absolute h-[181.00px] w-[146.00px] left-[calc(50%-72.94px)] top-[calc(50%-90.92px)]"
                  style={{
                    background:
                      "url(/assets/faq-detailsmob/images/image_26.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>

            <div
              id="_266_887__Frame_309"
              className="absolute h-[46.34px] w-[235.98px] left-[calc(50%-117.50px)] top-[285.00px]"
            >
              <div
                id="_266_888__Link"
                onClick={() => navigate('/contact')}
                className="absolute bg-[rgba(1,97,254,1.00)] h-[46.34px] w-[126.33px] left-[calc(50%-117.99px)] top-[calc(50%-23.17px)] rounded-[95.3499984741211px]"
              >
                <div
                  id="_266_889__Container"
                  className="absolute overflow-hidden h-[calc(100%-6.09px)] w-[50.08px] left-[calc(50%-40.28px)] top-[3.05px]"
                >
                  <span
                    id="_266_890__Contact"
                    className="flex justify-center text-center items-center h-[16.00px] w-[50.74px] absolute left-[calc(50%-25.04px)] top-[calc(50%+52.32px)]"
                  >
                    <span
                      className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[13.399999618530273px] font-medium leading-[20.14px] tracking-[-0.06700000166893005px]"

                    >
                      Contact
                    </span>
                  </span>
                </div>

                <div
                  id="_266_891__Frame_254"
                  className="absolute h-[22.88px] w-[80.62px] flex flex-row justify-start items-center flex-nowrap gap-[7px] left-[22.88px] top-[11.74px]"
                >
                  <span
                    id="_266_892__Contact"
                    className="flex justify-center text-center items-center h-[16.00px] w-[50.74px] relative"
                  >
                    <span
                      className="whitespace-nowrap bg-white bg-clip-text text-transparent not-italic text-[14.0px] font-medium leading-[20.14px] tracking-[-0.06700000166893005px]"

                    >
                      Contact
                    </span>
                  </span>
                  <div
                    id="_266_893__67a22e78f02af13d518a"
                    className="relative overflow-hidden h-[22.88px] w-[22.88px]"
                  >
                    <div
                      id="_266_894__67a22e78f02af13d518a"
                      className="absolute overflow-hidden h-[22.88px] w-[22.88px] left-[0.00px] top-[0.00px]"
                    >
                      <div
                        id="_266_895__67a22e78f02af13d518a"
                        className="absolute overflow-hidden h-[22.88px] w-[22.88px] left-[calc(50%-11.44px)] top-[calc(50%-11.44px)]"
                      >
                        <img
                          id="_266_896__Vector"
                          src="/assets/faq-detailsmob/images/vector_11.svg"
                          alt="Vector"
                          className="absolute left-[calc(100%_*_0.17)] top-[calc(100%_*_0.25)]"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              <div
                id="_266_897__Link"
                onClick={() => navigate('/portfolio1')}
                className="absolute border h-[42.63px] w-[100.03px] top-[calc(50%-22.31px)] rounded-[95.3499984741211px] border-[#0161feff] border-solid left-[calc(50%+15.96px)]"
              >
                <span
                  id="_266_898__Portfolio"
                  className="flex justify-center text-center items-center h-[17.00px] w-[50.00px] absolute left-[calc(50%-25.34px)] top-[calc(50%-8.37px)]"
                >
                  <span
                    className="whitespace-nowrap bg-[rgba(1,97,254,1.00)] bg-clip-text text-transparent not-italic text-[14.0px] font-medium leading-[19.17px] tracking-[-0.06700000166893005px]"

                  >
                    Portfolio
                  </span>
                </span>
              </div>
            </div>

            <div
              id="_266_899__Container"
              className="absolute overflow-hidden h-[289.64px] w-[274.39px] rounded-[15.260000228881836px] border-[rgba(142,131,131,0.20)] border-solid border-[1px] left-[233.48px] top-[361.00px]"
            >
              <div
                id="_266_900__67b5af0b7a74e5519cee"
                className="absolute overflow-hidden bg-[linear-gradient(0deg,rgba(255,255,255,1.00)0%,rgba(255,255,255,1.00)100%)] h-[100.00%] w-[100.00%]"
              >
                <div
                  id="_266_901__image_10"
                  className="absolute h-[129.00px] w-[184.00px] left-[calc(50%-91.78px)] top-[calc(50%-64.92px)]"
                  style={{
                    background:
                      "url(/assets/faq-detailsmob/images/image_10.png) 100% / cover no-repeat",
                  }}
                ></div>
              </div>
            </div>
          </div>

          {/* Footer */}
          <div className="absolute w-[calc(100%-16px)] left-[8px]" style={{ top: 3897 }}>
            <MobFooter />
          </div>
        </div>

      </div>{/* end mobile view */}

    </>
  );
};
export default FAQDetailsDesktop;


